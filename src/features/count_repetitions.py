...
bench_df = df[df["label"] == "Flat-bench-press"]
ohp_df = df[df["label"] == "Arnold-press"]
squat_df = df[df["label"] == "Squats"]
dead_df = df[df["label"] == "Romanian-deadlift"]
pull_df = df[df["label"] == "Lat-Pulldown"]
...
pull_set = pull_df[pull_df["set"] == pull_df["set"].unique()[0]]
...
count_reps(ohp_set, cutoff=0.2)
count_reps(squat_set, cutoff=0.2)
count_reps(bench_set, cutoff=0.2)
count_reps(dead_set, cutoff=0.2)
count_reps(pull_set, cutoff=0.2)
