...
predictor_columns = list(df.columns[:3])
...
subset[["acc_r"]].plot(subplots=True)
...
predictor_columns = predictor_columns + ["acc_r"]
...
df_freq = pd.concat(df_freq_list).set_index("Timestamp (ms)", drop=True)
...
df_cluster.to_pickle("../../data/interim/03_linAccData_features.pkl")
df_cluster.to_csv("../../data/interim/03_linAccData_features.csv")
